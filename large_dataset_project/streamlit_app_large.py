...
st.set_page_config(
    page_title="Insurance Premium Predictor | Large Dataset",
    page_icon="🏥",
    layout="wide",
...
def load_model():
    try:
        model      = pickle.load(open('best_model_large.pkl', 'rb'))
        encoders   = pickle.load(open('encoders_large.pkl', 'rb'))
        model_info = pickle.load(open('model_info_large.pkl', 'rb'))
...
def load_data():
    try:
        return pd.read_csv('large_insurance_50000.csv')
    except FileNotFoundError:
...
def get_model_results():
    return pd.DataFrame({
        ...
        'Accuracy (%)' : [93.11, 91.85, 88.42, 88.39, 88.40, 79.34],
        'Error ($)'    : [1974, 2089, 2601, 2603, 2602, 2741],
        'RMSE ($)'     : [2468, 2701, 3198, 3201, 3199, 4312],
...
with st.sidebar:
    ...
    st.markdown("*Large Dataset — 50K Records*")
    st.markdown("---")
...
if page == "🏠 Home":
    ...
    with c1:
        st.markdown('<div class="metric-card"><h2>📊 Data-Driven</h2><p>50,000 real insurance records</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="metric-card"><h2>🎯 Accurate</h2><p>93%+ prediction accuracy</p></div>', unsafe_allow_html=True)
    with c3:
...
# ── ANALYSIS ──────────────────────────────────────────────────
    ...
    corr = df_enc.corr()
    smoker_corr = corr['charges']['smoker']
    fig = px.imshow(corr, text_auto='.2f', aspect='auto',
                    color_continuous_scale='RdBu_r',
                    title=f'Correlation Matrix — smoker has strongest link to charges ({smoker_corr:.2f})')
    ...
    # 4. Summary table — dynamic from actual data
    st.markdown("### 📊 Feature Importance Summary")
    corr_vals = corr['charges'].drop('charges')
    summary = pd.DataFrame({
        'Feature'     : ['🚬 Smoker','📅 Age','⚖️ BMI','👶 Children','👤 Gender','🗺️ Region'],
        'Correlation' : [
            round(corr_vals.get('smoker', 0), 2),
            round(corr_vals.get('age', 0), 2),
            round(corr_vals.get('bmi', 0), 2),
            round(corr_vals.get('children', 0), 2),
            round(corr_vals.get('sex', 0), 2),
            round(corr_vals.get('region', 0), 2),
        ],
        'Impact Level': ['Very High','Moderate','Moderate','Low','Very Low','Negligible'],
        'Insight'     : [
            f'Smokers pay {pct:.0f}% more',
            'Each decade adds to premium',
...
# ── MODELS ────────────────────────────────────────────────────
    ...
    with c1:
        ...
        **🏆 Why Gradient Boosting Won:**
        - Highest R² score: 93.11%
        - Handles complex non-linear relationships
        - Robust to outliers in data
        - Sequential error correction
        - Scales well with 50K records
        """)
    with c2:
        st.info("""
        **📈 Key Patterns:**
    ...
    4. **Selected** Gradient Boosting — best accuracy at 93.11% with $1,974 avg error on 50K records
    """)
...
# ── ABOUT ─────────────────────────────────────────────────────
    ...
    with c1:
        ...
        <div style='background:linear-gradient(135deg,#667eea,#764ba2);
            ...
            <div style='font-size:1.5rem;font-weight:800;'>50,000</div>
    ...
    with c2:
        ...
        <div style='background:linear-gradient(135deg,#11998e,#38ef7d);
                    border-radius:12px;padding:1.2rem;text-align:center;color:white;'>
            <div style='font-size:2rem;'>🎯</div>
            <div style='font-size:1.5rem;font-weight:800;'>93.11%</div>
    ...
    with c4:
        ...
        <div style='background:linear-gradient(135deg,#fc4a1a,#f7b733);
                    border-radius:12px;padding:1.2rem;text-align:center;color:white;'>
            <div style='font-size:2rem;'>💰</div>
            <div style='font-size:1.5rem;font-weight:800;'>$1,974</div>
    ...
    with c1:
        ...
        <div style='background:white;border-radius:14px;padding:1.5rem;
            ...
            • <b>50,000 records</b> from real insurance customers<br>
            • <b>6 input features:</b> Age, Sex, BMI, Children, Smoker, Region<br>
            • <b>Target:</b> Annual insurance charges in USD<br>
            • <b>Source:</b> Large Insurance Dataset (50K)
            </p>
        </div>""", unsafe_allow_html=True)
