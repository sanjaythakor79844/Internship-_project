...
def load_model():
    try:
        ...
        st.error(f"ΓÜá∩╕Å Error loading model: {e}")
...
def load_data():
    try:
        ...
        st.error(f"ΓÜá∩╕Å Dataset not found: {e}")
        return None
...
# ΓöÇΓöÇ DASHBOARD ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    with c2:
        ...
        fig = px.bar(children_avg, x='children', y='charges',
                     title='≡ƒæ╢ Avg Premium by No. of Children',
...
# ΓöÇΓöÇ PREDICT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    if st.button("🔮 Predict My Premium", type="primary"):
        ...
        st.markdown(f'<div class="prediction-box">≡ƒÆ░ Predicted Annual Premium: ${prediction:,.2f}</div>',
                    unsafe_allow_html=True)

        st.markdown("### ≡ƒÆí Tips to Lower Your Premium:")
        c1, c2 = st.columns(2)
        with c1:
            if smoker == 'yes':
            ...
            if bmi > 30:
                st.info("≡ƒÅâ Reducing BMI below 30 can lower your premium.")
            elif bmi <= 25:
...
# ΓöÇΓöÇ ANALYSIS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    st.markdown("### ≡ƒöì Feature Correlation Heatmap")
    ...
    fig = px.imshow(corr, text_auto='.2f', aspect='auto',
                    ...
                    title='Correlation Matrix ΓÇö smoker has strongest link to charges (0.79)')
    ...
    c1, c2 = st.columns(2)
    with c1:
        # Box plot ΓÇö distribution spread
    ...
    with c2:
        # Violin plot ΓÇö full distribution shape
    ...
    # 3. Age & BMI effect
    st.markdown("### ≡ƒôà Age & BMI Effect on Premiums")
    c1, c2 = st.columns(2)
    with c1:
        # Age group analysis
...
# ΓöÇΓöÇ MODELS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    if model_info:
        st.markdown("### ≡ƒÅå Best Model Performance")
        c1, c2, c3 = st.columns(3)
    ...
    st.markdown("### ≡ƒöì My Model Selection Process")
    st.markdown("""
    ...
    4. **Selected** Gradient Boosting ΓÇö best accuracy at 87.67% with $2,425 avg error
    """)
...
# ΓöÇΓöÇ ABOUT ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    with c4:
        ...
        <div style='background:linear-gradient(135deg,#fc4a1a,#f7b733);
                    border-radius:12px;padding:1.2rem;text-align:center;color:white;'>
            <div style='font-size:2rem;'>≡ƒÆ░</div>
    ...
    with c1:
        ...
        <div style='background:white;border-radius:14px;padding:1.5rem;
            ...
            I built this to understand how data science can solve real business problems ΓÇö
        ...
        <div style='background:white;border-radius:14px;padding:1.5rem;
                    box-shadow:0 2px 12px rgba(0,0,0,0.08);margin-bottom:1rem;'>
            <h3 style='color:#1f77b4;margin-top:0;'>≡ƒ¢á∩╕Å Tech Stack</h3>
        ...
        <div style='background:white;border-radius:14px;padding:1.5rem;
            ...
            ΓÇó <b>Target:</b> Annual insurance charges in USD ($1,121 ΓÇô $63,770)<br>
            ΓÇó <b>Source:</b> Kaggle Insurance Dataset
            </p>
        </div>""", unsafe_allow_html=True)
    ...
    with c2:
        ...
        <div style='background:linear-gradient(135deg,#11998e,#38ef7d);
            ...
            ≡ƒÆí This project taught me how ML can solve real business problems!
            </div>
        </div>""", unsafe_allow_html=True)
